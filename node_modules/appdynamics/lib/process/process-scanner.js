...
ProcessScanner.prototype.startAutoSnapshotIfPossible = function (callback) {
  ...
  self.startSnapshot(true, self.autoSnapshotDurationSeconds, -1, ensureCallback(self, callback));
}
...
ProcessScanner.prototype.startManualSnapshot = function(processCallGraphReq, callback) {
  ...
  self.startSnapshot(false, processCallGraphReq.captureTime, requestID, ensureCallback(self, callback));
}
      ...
      if(!auto) {
        try {
          if(self.agent.heapProfiler.isObjectTrackingSupported()) {
            self.agent.logger.log("ProcessScanner - Starting heap allocation tracking...");
            ...
            self.agent.logger.log("ProcessScanner - Heap allocation tracking is not supported.");
          }
        }
        catch(err) {
          self.agent.logger.error("ProcessScanner - Heap allocation tracking failed.");
          self.agent.logger.error(err);
        }
      }
